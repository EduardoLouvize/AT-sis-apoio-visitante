  ...
  methods: {
    salvarSenha: function salvarSenha() {
      if (this.user == "" || this.senha == "" || this.senhaConfirm == "") {
        ...
        String(this.user).length < 5 ||
        String(this.senha).length < 5 ||
        String(this.senhaConfirm).length < 5
      ) {
        this.alerta.error = true;
        this.alerta.msg = "Insira no minimo 5 caracteres";
        this.alerta.bgc.backgroundColor = "red";
      } else if (
        String(this.user).length > 10 ||
        String(this.senha).length > 10 ||
        String(this.senhaConfirm).length > 10
      ) {
        this.alerta.error = true;
        ...
        this.$store.dispatch("salvarSenha", {
          user: this.user,
          senha: this.senha,
        });
